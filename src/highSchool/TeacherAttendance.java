...
public class TeacherAttendance extends JFrame implements ActionListener {
	...
	TeacherAttendance() {
		super("Presença dos(a) professores(a)");
		jp = new JPanel();
		setSize(500, 350);
		jp.setLayout(null);
		...
		"Presença dos(a) professores(a)", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(55, 0, 179)));
		add(jp);
		
		l1 = new JLabel("Selecione o ID do(a) professor(a)");
		l1.setBounds(18, 50, 250, 20);
	...
	public void actionPerformed(ActionEvent actionEvent) {
		try {
			if (actionEvent.getSource() == b1) {
				try {
					String emp_id = c1.getSelectedItem();
					String date = new java.util.Date().toString();
					String first = c2.getSelectedItem();
					String second = c3.getSelectedItem();
					
					String query = "INSERT INTO attendance_teacher VALUES('"
							+ emp_id + "', '" + date + "', '" + first 
							+ "', '" + second + "')";
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TeacherAttendance frame = new TeacherAttendance();
